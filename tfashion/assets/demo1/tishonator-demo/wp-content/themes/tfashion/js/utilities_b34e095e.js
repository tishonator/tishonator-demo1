...
function initMiniCartEvents() {
  ...
  jQuery('#header-top a.cart-contents').on('mouseenter', function(){
    
    // display mini-cart if it's not visible
    if ( jQuery('#header-top #cart-popup-content').css('z-index') == '-1' ) {
    
      var rightPos = (jQuery(window).width() - (jQuery(this).offset().left + jQuery(this).outerWidth()));
      var topPos = jQuery(this).offset().top + 20;
    
      jQuery('#header-top #cart-popup-content').css('z-index', '5000').css('opacity', '1').css('right', rightPos).css('top', topPos);
    }
  });
  
  jQuery('#header-top #cart-popup-content').on('mouseleave', function(){
  
    jQuery('#header-top #cart-popup-content').css('z-index', '-1').css('opacity', '0');
  });
}
